...
Result<Texture, Texture::CreateError> Texture::createFromImage(const Image &image)
{
	...
	std::cout << image.filepath().c_str();
	unsigned char *data = stbi_load("../res/textures/cy.jpg", &texWidth, &texHeight, &texChannels, 4);
	if (data)
	{
	...
	return Result::Ok(Texture(textureId, texWidth, texHeight));
