...
# List of coordinates to process (starting from the center and moving outward)
coordinates_to_process = [(center_x, center_y)]

# To generate outward spiral, we use layers of increasing radius
radius = 1

while len(coordinates_to_process) < width * height:
    for dx in range(-radius, radius + 1):
        for dy in range(-radius, radius + 1):
            # Ensure we're within bounds of the image
            nx, ny = center_x + dx, center_y + dy
            if 0 <= nx < width and 0 <= ny < height and (nx, ny) not in tile_id_map:
                coordinates_to_process.append((nx, ny))

    radius += 1  # Expand the radius to cover the next layer

# Now process each coordinate in the order of the outward spiral
for x, y in coordinates_to_process:
    ...
    # For each direction (up, down, left, right), process neighbors
...
# === Save the map ===
with open("Small_Map_non_euc.json", "w") as f:
    json.dump(data, f, indent=2)

print(f"Generated {len(data)} tiles.")
