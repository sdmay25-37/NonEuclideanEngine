...
private:
	static void UpdateTile2(entt::registry &registry, Resource<TextureManager> texture_manager,
	                        Resource<TileMap> tilemap, Resource<RendererHyp> renderer, Resource<Window> window) {
	...
	static void CreateTiles3(entt::registry &registry, Resource<TextureManager> texture_manager,
	...
	static void addTileAndNeighborsBFS2(entt::registry &registry, Resource<TextureManager> texture_manager,
	                                    ...
	                                    Resource<TileMap> tilemap, std::unordered_set<int> *processed_tiles,
	                                    float ThetaX, float ThetaY) {
		...
		while(!tile_queue.empty()) {
			...
			if(currentRelation >= radius || processed_tiles->find(tile._tileId) != processed_tiles->end() || tile.
			   _tileId == -1) {
	...
	addTileAndNeighbors(entt::registry &registry, Resource<TextureManager> texture_manager,
	                    ...
	                    std::unordered_set<int> processed_tiles, float ThetaX, float ThetaY) {
		...
		if(root_tile._leftTileId != -1 && tilemap->getTileInRenderedList(root_tile._leftTileId)._tileId != -1) {
			...
			addTileAndNeighbors(registry, texture_manager, tile_left, currentRelation + 1, radius, tile_left.sprite,
			                    left_tile, tilemap, processed_tiles, ThetaX, newThetaY);
		}
		if(root_tile._rightTileId != -1 && tilemap->getTileInRenderedList(root_tile._downTileId)._rightTileId != -1) {
			...
			addTileAndNeighbors(registry, texture_manager, tile_right, currentRelation + 1, radius, tile_right.sprite,
			                    right_tile, tilemap, processed_tiles, ThetaX, newThetaY);
		}
		if(root_tile._upTileId != -1 && tilemap->getTileInRenderedList(root_tile._upTileId)._tileId != -1) {
			...
			addTileAndNeighbors(registry, texture_manager, tile_top, currentRelation + 1, radius, tile_top.sprite,
			                    top_tile, tilemap, processed_tiles, newThetaX, ThetaY);
		}
		if(root_tile._downTileId != -1 && tilemap->getTileInRenderedList(root_tile._downTileId)._tileId != -1) {
			...
			float newThetaX = ThetaX - Theta;
			addTileAndNeighbors(registry, texture_manager, tile_bottom, currentRelation + 1, radius, tile_bottom.sprite,
			                    bottom__tile, tilemap, processed_tiles, newThetaX, ThetaY);
		}
	}
	...
	static void MoveCamera(Resource<Camera> camera, Resource<Input> input, Resource<TileMap> tilemap,
	                       entt::registry &registry, Resource<TextureManager> texture_manager,
	                       Resource<RendererHyp> renderer, Resource<Window> window) {
		...
		if(timeSinceLastMove < moveCooldown)
			...
			if(tilemap->currentTile._upTileId != -1 && isValidTileToMove(
				   tilemap->getTileInRenderedList(tilemap->currentTile._upTileId), tilemap)) {
			...
			// std::cout << tilemap->currentTile.to_string() << "\n";
			if(tilemap->currentTile._leftTileId != -1 && isValidTileToMove(
				   tilemap->getTileInRenderedList(tilemap->currentTile._leftTileId), tilemap)) {
			...
			// std::cout << tilemap->currentTile.to_string() << "\n";
			if(tilemap->currentTile._downTileId != -1 && isValidTileToMove(
				   tilemap->getTileInRenderedList(tilemap->currentTile._downTileId), tilemap)) {
			...
			// std::cout << tilemap->currentTile.to_string() << "\n";
			if(tilemap->currentTile._rightTileId != -1 && isValidTileToMove(
				   tilemap->getTileInRenderedList(tilemap->currentTile._rightTileId), tilemap)) {
